...
from src.llm_model_registry import capability_summary
...
class SOPGenerationService:
    def __init__(
        ...
        self._atomizer = SOPLLMAtomizer(llm=llm, on_progress=on_progress)
        self._last_atomize_result: Any | None = None

    def set_progress_callback(self, callback: Any | None) -> None:
        self._on_progress = callback
        self._atomizer = SOPLLMAtomizer(llm=self._llm, on_progress=callback)

    def generate_from_document(self, file_path: str | Path) -> Dict[str, Any]:
        extraction = self._ingestor.extract_document(file_path)
        atomize_result = self._atomizer.atomize(extraction)
        self._last_atomize_result = atomize_result
        steps = list(atomize_result.steps)
        questions = self._build_questions(
            steps, extraction, atomize_result.coverage_report
        )
        canonical = self._build_canonical(extraction, steps, questions, {})
        canonical["atomization"] = {
            "mode": atomize_result.atomization_mode,
            "warnings": list(atomize_result.warnings),
            "chunk_count": atomize_result.chunk_count,
            "coverage_report": atomize_result.coverage_report.to_json(),
        }
        compile_result = self.compile_to_runtime_json(
            canonical, self.build_runtime_profile()
        )
        canonical["compile_result"] = self._compile_payload(compile_result)
        self._append_generation_audit(canonical, file_path)
        return canonical

    def generation_readiness(self) -> str:
        if self._llm is None:
            raise RuntimeError(
                "SOP Generate requires a configured Ollama runtime with sop_generation model."
            )
        health_check = getattr(self._llm, "check_sop_generation_health", None)
        if callable(health_check):
            return health_check()
        cfg = getattr(self._llm, "cfg", None)
        if cfg is None:
            raise RuntimeError("SOP Generate LLM config is missing.")
        sop_cfg = (
            cfg.sop_generation_cfg() if hasattr(cfg, "sop_generation_cfg") else None
        )
        model_path = getattr(sop_cfg, "model_path", "qwen3:8b")
        summary = capability_summary(str(model_path))
        return f"SOP Generate configured for {model_path} ({summary.get('role', 'custom')})"
    ...
    def answer_generation_questions(
        ...
        question_map = {
            q["id"]: q
            for q in updated.get("questions_asked", [])
        ...
        compile_result = self.compile_to_runtime_json(
            updated, self.build_runtime_profile()
        )
    ...
    def finalize_canonical_sop(self, canonical: Dict[str, Any]) -> Dict[str, Any]:
        ...
        coverage = finalized.get("atomization", {}).get("coverage_report", {})
        unmapped = coverage.get("unmapped_refs") or []
        if unmapped:
            labels = [
                str(ref.get("label") or ref.get("index") or "unknown")
                for ref in unmapped
                if isinstance(ref, dict)
            ]
            raise ValueError(
                "Cannot finalize: unmapped source sections remain: " + ", ".join(labels)
            )
        missing = []
        for question in finalized.get("questions_asked", []):
            if not isinstance(question, dict):
                continue
            if question.get("required") and question.get("id") not in finalized.get(
                "answers", {}
            ):
        ...
        compile_result = self.compile_to_runtime_json(
            finalized, self.build_runtime_profile()
        )
    ...
    def compile_to_runtime_json(
        ...
        for step in canonical.get("workflow", {}).get("steps", []):
            ...
            if target_name and registry.get_type(target_name) is None:
                warnings.append(
                    f"Runtime target '{target_name}' is not present in class_registry.json"
                )

        if not runtime_steps:
            warnings.append(
                "No automatable runtime steps were produced from the canonical SOP."
            )
        ...
        with zipfile.ZipFile(dest, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            if compile_result is not None:
                zf.writestr(
                    ...
                    json.dumps(
                        compile_result.runtime_json, ensure_ascii=False, indent=2
                    ),
                )
        return dest
    ...
    def import_sop_package(self, package_path: str | Path) -> Dict[str, Any]:
        ...
        with zipfile.ZipFile(path, "r") as zf:
            ...
            if "compiled.sop_steps.json" in zf.namelist():
                compiled = json.loads(
                    zf.read("compiled.sop_steps.json").decode("utf-8")
                )
        return {
            "manifest": manifest,
            "canonical": canonical,
    ...
    def _extract_step_candidates(
        self, extraction: SOPDocumentExtraction
    ) -> List[Dict[str, Any]]:
        candidates: List[Dict[str, Any]] = []
        blocks = extraction.refs or self._ingestor._build_text_refs(
            extraction.raw_text
        )  # noqa: SLF001
    ...
    def _build_questions(
        ...
        if coverage_report is not None:
            for ref in getattr(coverage_report, "unmapped_refs", []) or []:
                if not isinstance(ref, dict):
                    continue
                label = str(ref.get("label") or ref.get("index") or "section")
                questions.append(
                    {
                        "id": f"unmap_{ref.get('kind')}_{ref.get('index')}",
                        "category": "coverage_gap",
                        "prompt": f"How should unmapped section '{label}' be handled?",
                        "answer_type": "single_choice",
                        "options": ["automatable", "manual", "skip"],
                        "required": True,
                        "affects_fields": [],
                        "source_refs": [ref],
                    }
                )
            for step_id in getattr(coverage_report, "low_confidence_steps", []) or []:
                questions.append(
                    {
                        "id": f"{step_id}_confidence_review",
                        "category": "low_confidence",
                        "prompt": f"Please confirm automation for low-confidence step '{step_id}'.",
                        "answer_type": "single_choice",
        ...
        for step in steps:
            ...
            if not step.get("target") and step.get("automation_kind") in {
                "automatable",
                "conditional",
            }:
                questions.append(
            ...
            if step.get("action_kind") == "wait" and "ms" not in step.get(
                "parameters", {}
            ):
                questions.append(
                    {
    ...
    def _compile_step(
        ...
        if automation_kind not in _AUTOMATION_KINDS or automation_kind not in {
            "automatable",
            "conditional",
        }:
            return None
        ...
        if action_kind == "click":
            ...
            if runtime_target:
                ...
                compiled["text"] = str(
                    parameters.get("text") or parameters.get("value") or "0"
                )
                compiled["clear_first"] = bool(parameters.get("clear_first", True))
            else:
                runtime_type = "type_text"
                compiled["text"] = str(
                    parameters.get("text") or parameters.get("value") or ""
                )
            ...
            compiled["password_field"] = str(
                parameters.get("password_field") or "password_field"
            )
    ...
    def _set_nested(
        self, root: Dict[str, Any], path: Sequence[str], value: Any
    ) -> None:
        cur: Any = root
        for part in path[:-1]:
    ...
    def dry_run_compile(self, canonical: Dict[str, Any]) -> RuntimeCompileResult:
        return self.compile_to_runtime_json(canonical, self.build_runtime_profile())

    def _append_generation_audit(
        self, canonical: Dict[str, Any], file_path: str | Path
    ) -> None:
        log_path = Path("logs/sop_generation_audit.jsonl")
        try:
            log_path.parent.mkdir(parents=True, exist_ok=True)
            entry = {
                "source_path": str(file_path),
                "title": canonical.get("metadata", {}).get("title"),
                "atomization": canonical.get("atomization", {}),
                "step_count": len(canonical.get("workflow", {}).get("steps", [])),
            }
